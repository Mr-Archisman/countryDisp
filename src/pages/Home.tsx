...
const Home = () => {
  ...
  async function fetchMoreCountries() {
    ...
    const newUniqueCountries = newCountries.filter(
      (newCountry: { name: { common: string } }) =>
        !countries.some(
          (existingCountry) =>
  ...
  return (
    ...
    <div className={darkMode ? 'dark' : ''}>
      ...
      <div className='bg-[#FAFAFA] dark:bg-[#202C36] p-4 md:p-12 2xl:px-64 mt-16'>
        <div className='flex justify-between  items-center'>
          <div className='relative w-[60vw]'>
            ...
            <svg
              ...
              className='h-5 w-5 absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-500 dark:text-white'
              fill='none'
              viewBox='0 0 24 24'
              stroke='currentColor'
            >
          ...
          >
            ...
            {regions.map((region) => (
              <option key={region} value={region} className='dark:text-white'>
                {region}
              </option>
            ))}
          </select>
